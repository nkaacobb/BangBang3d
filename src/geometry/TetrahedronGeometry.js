import { BufferGeometry } from './BufferGeometry.js';
import { BufferAttribute } from './BufferAttribute.js';

/**
 * TetrahedronGeometry - Creates a tetrahedron (4 triangular faces)
 * One of the five Platonic solids
 */
export class TetrahedronGeometry extends BufferGeometry {
  constructor(radius = 1) {
    super();

    this.type = 'TetrahedronGeometry';
    this.parameters = { radius };

    // Tetrahedron vertices (regular tetrahedron centered at origin)
    const a = 1 / Math.sqrt(3);
    const vertices = [
      // v0: top
       0,  a * Math.sqrt(2),  0,
      // v1: front
       0,  0,  a * Math.sqrt(6) / 3,
      // v2: back-left
      -a,  0, -a * Math.sqrt(6) / 6,
      // v3: back-right
       a,  0, -a * Math.sqrt(6) / 6
    ];

    // Scale vertices by radius
    for (let i = 0; i < vertices.length; i++) {
      vertices[i] *= radius;
    }

    // Indices for 4 faces (each is a triangle)
    const indices = [
      2, 1, 0,  // front-left face
      3, 2, 0,  // back face
      1, 3, 0,  // front-right face
      2, 3, 1   // bottom face
    ];

    // Calculate normals for each face
    const normals = [];
    const uvs = [];

    // For each face, calculate face normal and assign UVs
    for (let i = 0; i < 4; i++) {
      const i0 = indices[i * 3 + 0];
      const i1 = indices[i * 3 + 1];
      const i2 = indices[i * 3 + 2];

      // Get vertices
      const v0 = [vertices[i0 * 3], vertices[i0 * 3 + 1], vertices[i0 * 3 + 2]];
      const v1 = [vertices[i1 * 3], vertices[i1 * 3 + 1], vertices[i1 * 3 + 2]];
      const v2 = [vertices[i2 * 3], vertices[i2 * 3 + 1], vertices[i2 * 3 + 2]];

      // Calculate face normal
      const edge1 = [v1[0] - v0[0], v1[1] - v0[1], v1[2] - v0[2]];
      const edge2 = [v2[0] - v0[0], v2[1] - v0[1], v2[2] - v0[2]];
      
      const normal = [
        edge1[1] * edge2[2] - edge1[2] * edge2[1],
        edge1[2] * edge2[0] - edge1[0] * edge2[2],
        edge1[0] * edge2[1] - edge1[1] * edge2[0]
      ];

      // Normalize
      const len = Math.sqrt(normal[0] * normal[0] + normal[1] * normal[1] + normal[2] * normal[2]);
      normal[0] /= len;
      normal[1] /= len;
      normal[2] /= len;

      // Assign same normal to all 3 vertices of the face (flat shading)
      for (let j = 0; j < 3; j++) {
        normals.push(normal[0], normal[1], normal[2]);
      }
    }

    // Assign UVs (simple mapping for texturing)
    // Each face gets standard triangle UVs
    for (let i = 0; i < 4; i++) {
      uvs.push(0.5, 0, 1, 1, 0, 1);
    }

    // Expand vertices array to match indexed drawing
    const expandedVertices = [];
    for (let i = 0; i < indices.length; i++) {
      const idx = indices[i];
      expandedVertices.push(
        vertices[idx * 3],
        vertices[idx * 3 + 1],
        vertices[idx * 3 + 2]
      );
    }

    // Build geometry
    this.setIndex(Array.from({ length: indices.length }, (_, i) => i));
    this.setAttribute('position', new BufferAttribute(new Float32Array(expandedVertices), 3));
    this.setAttribute('normal', new BufferAttribute(new Float32Array(normals), 3));
    this.setAttribute('uv', new BufferAttribute(new Float32Array(uvs), 2));
  }
}
